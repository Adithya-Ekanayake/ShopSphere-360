  ...
  return (
    <>
      ...
      <div className="advanced-grid">
        ...
        <AdvancedSection title="Retention & Repeat Purchase" description="Repeat rate measures multiple orders in a month; retention measures customers who repurchased within 180 days of their previous order." state={sections.retention}>
          <div className="advanced-stat-row"><strong>{latestRetention ? `${Number(latestRetention.RetentionRatePercent ?? 0).toFixed(1)}%` : "—"}</strong><span>latest 180-day repurchase rate</span><strong>{latestRetention ? `${Number(latestRetention.RepeatCustomerRatePercent).toFixed(1)}%` : "—"}</strong><span>latest repeat rate</span></div>
          <div className="advanced-chart-wrap"><ResponsiveContainer width="100%" height={280}><LineChart data={retentionTrend}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--chart-grid)" /><XAxis dataKey="PeriodLabel" tick={{ fontSize: 10, fill: "var(--chart-axis-text)" }} /><YAxis unit="%" tick={{ fontSize: 11, fill: "var(--chart-axis-text)" }} /><Tooltip formatter={(value) => `${Number(value ?? 0).toFixed(1)}%`} /><Line type="monotone" dataKey="RepeatCustomerRatePercent" name="Repeat rate" stroke="var(--primary)" strokeWidth={3} dot={false} /><Line type="monotone" dataKey="RetentionRatePercent" name="180-day repurchase" stroke="#0f766e" strokeWidth={3} dot={false} /></LineChart></ResponsiveContainer></div>
        </AdvancedSection>
